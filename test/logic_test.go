...
func TestProcessShows(t *testing.T) {
	//Test 1: Happy Path
	...
	retVal,err := model.ProcessShows(test1)
	...
	for index, show := range retVal.Shows {
		...
		if (img != "imageTest1") {
			 t.Error("Test1: Expected imageTest1 got", img)
		}
		if (slug != "SlugTest1") {
			 t.Error("Test1: Expected SlugTest1 got", slug)
		}
		if (title != "Test1") {
			 t.Error("Test1: Expected Test1 got", title)
		}
		if (index > 1) {
			 t.Error("Test1: More then one show returned:", index)
		}
	}
}
